...
const productElem = document.querySelector(".products");
...
function percent(item) {
    let percentageSal = item.price / 100 * item.discountPercentage
    let endPrice = item.price - percentageSal;
    return endPrice;
};
function render(renderedList) {
    productElem.innerHTML = ""
    renderedList.forEach(item => productElem.innerHTML +=
        `<div class="product">
                    ...
                    <div class="productInfo">
                        ...
                        <span>qiymetine YOX!</span>
                        <span>${item.discountPercentage}% endirim ile </span>
                        <span>$${percent(item)}</span> 
                        <span>qiymetine elde edin!</span>
                        <h6>Rating: ${item.rating}</h6>
                        <h6>Stock: ${item.stock}</h6>
                        <button onclick="deleteProduct(${item.id})" class="deleteButton"> Delete </button>
                     </div>
         </div>`
    );
}
...
inputElem.addEventListener("keyup", () => {
    let filteredProduct = products.filter(item => {
        ...
        return brand.includes(inputValue);
    })
...
function deleteProduct(id) {
    console.log(id)
    let filterDeleteProduct = products.filter(item => item.id !== id);
    if (!filterDeleteProduct.length) {
        notFoundImage.style.display = "block";
    } else {
        notFoundImage.style.display = "none";
    }
    products = filterDeleteProduct;
    render(products);
}
...
//         if (item.brand.toLocaleLowerCase() === inputElem.value.toLocaleLowerCase()) {
...
// let newProducts = [];
// products.forEach(item => {
//     if (item.brand === inputElem.value) {
//         newProducts.push(item)
//     }
// })
// console.log(newProducts)

// newProducts.forEach(np => productElem.innerHTML +=
//     `<div class="product">
//             <img src="${np.thumbnail}" alt="product">
//             <div class="productInfo">
//                 <h1>${np.brand}</h1>
//                 <h2>${np.title}</h2>
//                 <p>${np.description}</p>
//                 <span>$${np.price}</span>
//                 <span>qiymetine YOX!</span>
//                 <span>${np.discountPercentage}% endirim ile </span>
//                 <span>$${percent(np)}</span>
//                 <span>qiymetine elde edin!</span>
//                 <h6>Rating: ${np.rating}</h6>
//                 <h6>Stock: ${np.stock}</h6>
//             </div>
//         </div>`)
// searchIcon.addEventListener("click", () => {
//     products.forEach(item => {
